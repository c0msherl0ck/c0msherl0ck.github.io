...
안드로이드 운영체제를 사용하는 핸드폰의 경우, adb(Android Debug Bridge)를 통해 backup 명령어를 입력함으로써 **논리적 이미지**를 획득하는 방법과 dd 명령어를 입력함으로써 **물리적 이미지**를 획득하는 방법이 있다. 이번 글에서는 **물리적 이미지** 획득을 위한 Rooting, BusyBox 설치 등 이미징 사전 준비단계부터 이미지 획득까지 전 과정에 대해 서술한다.
