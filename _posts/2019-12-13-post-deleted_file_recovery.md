...
`Winhex` 도구의 `File Recovery by Type` 기능을 이용하면 **$MFT 파일을 분석하지 않고도** 파일 복구가 가능하다(**단,** 폴더는 복구 대상이 아님). 해당 기능은 디스크 전체에서 파일 시그니처 기반의 파일 카빙을 통해 특정 파일 유형(Pictures, Documents, E-mail 등)을 선택하여 **다수의 파일들**을 복구하고, 그 중 찾는 파일이 있는지 **일일이 내용을 확인**하는 방식으로 진행한다(**단,** $MFT를 분석하지 않았기 때문에 파일 이름은 알 수 없음).
